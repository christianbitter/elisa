...
class Quaternion(object):
	...
	def Cross(self, q:Quaternion) -> Quaternion:
		if not q:
			raise ValueError("q not provided")		
		return Quaternion(self._y * q.z - self._z * q.y, self._z * q.x - self._x * q.z, self._x * q.y - self._y * q.x)
	...
	def __truediv__(self, q):
		...
		if is_numeric(q):
			qinv = 1. / q
			return Quaternion(self._x * qinv, self._y * qinv, self._z * qinv, self._w * qinv)
		elif isinstance(q, Quaternion):
			iq = q.inverse()
			return self * iq
		else:
