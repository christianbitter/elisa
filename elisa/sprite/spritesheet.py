...
def spritesheet_from_tiled(tsx_fp: str, **kwargs):
    ...
    if verbose:
        print(f"Spritesheet from tiled (v.{tiledVersion}")
    ...
    tileset_desc_json = {
        "name": name if j == 1 else f"{name}_{j}".format(j),
    ...
    return SpriteSheet(json_descriptor=tsx_fp).initialize_from_spritemap(
        tileset_desc_json, **kwargs
    )
