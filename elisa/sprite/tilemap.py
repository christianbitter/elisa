...
class TileMap(object):
	...
	def add_grid(self, grid_name:str, data:list, visible:bool=True, props:dict=None) -> TileMap:
	...
	def __repr__(self):
		return "TileMap[{}]({}, {}) - Tiles({}, {}); {} Layers".format(self._id,  self._map_dim[0], self._map_dim[1], self._tile_dim[0], self._tile_dim[1], len(self._grid))
...
def tilemap_from_tiled(tsm_fp:str, **kwargs) -> tuple:
	...
	for l in layers:
		layer_id   = l.attrib["id"]
		layer_name = l.attrib["name"]
		layer_width = int(l.attrib["width"])
		layer_height= int(l.attrib["height"])
		layer_visible = int(l.attrib.get("visible", 1)) != 0

		layer_custom_props = None

		if layer_width != width:
			raise ValueError("Layer cannot have different width than global tile map width")
		if layer_height != height:
			raise ValueError("Layer cannot have different height than global tile map height")

		props_node = l.findall("properties/*")
		if props_node and  len(props_node) > 0:
			layer_custom_props = {}
			for p in props_node:
				layer_custom_props[p.attrib["name"]] = p.attrib["value"]

		layer_data = l.findall("data")[0]
		layer_enc  = layer_data.get("encoding")
		if layer_enc != 'csv':
			raise ValueError("Encoding must be csv")
		grid_data = [int(v) for l in layer_data.text.splitlines() for v in l.split(",") if v.strip() != '']
