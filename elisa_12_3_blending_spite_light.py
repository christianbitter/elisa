...
# desc: using alpha blending and a simple gradient texture to achieve a simple lighting effect
#   - create light source
...
def main():
    ...
    l_img = light_sprite.image.convert()
    l_img.set_colorkey((0, 0, 0, 255))
    l_img.set_alpha(255)
    ...
    s1 = pygame.Surface((light_sprite.width, light_sprite.height))
    s1.set_alpha(255)
    s1.blit(l_img, (0, 0))
    ...
    while not is_done:
        ...
        l_x, l_y = x - l_w2, y - l_h2  # the coordinates of the light
        back_buffer.fill(C_BLACK)
