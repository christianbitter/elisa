...
class PlayerEntity(Entity):
	...
	def send_msg(self, msg):
		return {
			"UpdateTransform2D": self._handle_transform_message(msg)
...
class UpdateTransform2DMessage(Message):
	def __init__(self, translate_vec:Vec2=None, rotate_alpha:float=None, scale_vec:Vec2=None):
...
class RenderSystem(System):
	...
	def __init__(self, screen_width:int=640, screen_height:int=480):
		...
		self._screen_width = screen_height
		self._screen_buffer = pygame.display.set_mode(size=(screen_width, screen_height))
		self._back_buffer   = pygame.Surface(self._screen_buffer.get_size())
		self._back_buffer   = self._back_buffer.convert()
		self._cam2d         = Camera2D(ws_xmin=-screen_width, ws_xmax=screen_width, ws_ymin=-screen_height, ws_ymax=screen_height, 
																	 vw_height=screen_height, vw_width=screen_width)
	...
	def update(self, time_delta:float, entities:list):
		...
		for e in req_update:
			...
			eg_points = [e_mat * Vec3(p.x, p.y, 1.) for p in e_points]
...
def handle_key_pressed_event(kb_system):
	def handle_key_pressed(key_map, elapsed_time, entities):
		mov_speed_player_vert = 2.
		mov_speed_player_horz = 2.
		if key_map[pygame.K_UP]:
			for e in entities:
				e.send_msg(UpdateTransform2DMessage(translate_vec=Vec2(0., -mov_speed_player_horz)))
		...
		if key_map[pygame.K_LEFT]:
			for e in entities:
				e.send_msg(UpdateTransform2DMessage(translate_vec=Vec2(-mov_speed_player_vert, 0.)))
